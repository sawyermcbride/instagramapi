/**
* Sawyer McBride
* 8/6/17
* Instagram Follower App
*
*/
( function(){
	 
	const fs = require('fs');
	const request = require('./request.js');

	const options = {
		hostname:'www.instagram.com',
		path:'/graphql/query/?query_id=17851374694183129&variables=%7B%22id%22%3A%225669850849%22%2C%22first%22%3A20%7D',
		headers: {'Cookie': "sessionid=IGSC34abb732c40ee90c7e6d239dd16b7b1f519adad2575f1d246f293a02dfb61ae2%3AZd0nF5plkyc3Tkceq10SfP4iahPN3Gk9%3A%7B%22_auth_user_id%22%3A5669850849%2C%22_auth_user_backend%22%3A%22accounts.backends.CaseInsensitiveModelBackend%22%2C%22_auth_user_hash%22%3A%22%22%2C%22_token_ver%22%3A2%2C%22_token%22%3A%225669850849%3ALcEUTt79iJO1sOBsFIORdddgN3mv440h%3Ae701be51500916dc86f2120291972bd732304bfd299d75a0fb37c95bcc739125%22%2C%22_platform%22%3A4%2C%22last_refreshed%22%3A1502247940.1877083778%2C%22asns%22%3A%7B%22time%22%3A1502247992%2C%222602%3A30a%3A2eaf%3Ac7f0%3A751a%3Ad8d2%3A7c8d%3A5b43%22%3A7018%7D%7D;mid=WCkJFwAEAAGyt7gr87zT1tGkrBLl; csrftoken=lHTkZ1VrzHdM7uJcFQgK0SyFJvxNevw7; fbm_124024574287414=base_domain=.instagram.com; ig_vw=1102; ig_pr=1.25; fbsr_124024574287414=2Kc4nkf6R5h76Cp97oS8dVNIYbMwlcVpiuTF3x5DknA.; rur=ASH; csrftoken=dSK4Xl5XQ8vl86mjFi6nvfIU5N7aEzzs; ds_user_id=5669850849"
	 }
	}





	const loaded = new Promise( (resolve, reject) => {
		let i = 0;
		let users_all = [];
		function loadData () {
			i++;
			if(i > 10) {
				resolve(users_all);
				return;
			}
			let opts = options;
			request(options).then( (res) => {
				//console.log(res);
				let obj = JSON.parse( res );

				let hasNext = obj.data.user.edge_followed_by.page_info.has_next_page;
				let nextCursor = obj.data.user.edge_followed_by.page_info.end_cursor;

				let users = obj.data.user.edge_followed_by.edges;

				users_all.push(users);

				console.log(`Does it have a next? ${hasNext}. What is the cursor code? ${nextCursor}. How long is the users array? ${users.length}`);
				
				opts.path = `/graphql/query/?query_id=17851374694183129&variables=%7B%22id%22%3A%221481921906%22%2C%22first%22%3A11%2C%22after%22%3A%22${nextCursor}%22%7D`;

				loadData()
			});

			// console.log(users_all.length);
		}
		loadData();
	});

	loaded.then( (users) => {
		const objBuild = {users: []};
		users.forEach( (val) => {
			// console.log(val)
			objBuild.users.push(val);
		})
		let usersobj = {users:[]};
		for(let i = 0, a = {}; i < objBuild.users.length; i++) {
			a = objBuild.users[i];
			a.forEach( (val) => {
				console.log(val);
				usersobj.users.push( {
					username: val.node.username,
					id: val.node.id,
					fullname: val.node.full_name
				})
			});
		};
            	
		fs.writeFile('./output.txt', JSON.stringify(usersobj), (err) => {
			if(err) throw err;
			console.log("file written");
		})

	});
		
		
}())





